...
- Routes (all unauthenticated):
  ...
  - `POST /api/scenes/export` (live canvas) · `GET /api/scenes/{id}/export` (saved scene) — download a scene as a ZIP holding `scene.json` plus every upload it references; `POST /api/scenes/import` — multipart ZIP, restores the uploads and saves the scene under a fresh id ([archive.rs](backend/src/routes/archive.rs)). See [scenes.md](scenes.md)
...
- **3D projection (keystone):** Phaser 4 has no `Mesh`/`Plane`, so the warp is a CSS `matrix3d` written onto the game canvas by `_layoutCanvas`; the homography lives in [warp.ts](frontend/src/lib/phaser/warp.ts) (pure). Consequences worth remembering: Phaser maps pointers through `canvasBounds` *and* `displayScale`, both taken from the same bounding rect, so canvas input is disabled while warped and both values are recomputed by hand on the way back to flat (`scale.refresh()` would re-emit `RESIZE` and recurse). Corner handles are drawn in canvas space, so they are hidden while warped — under the warp they would land at `H(corner)`, not on the quad. The calibration grid is drawn twice-over depending on who carries the warp: when the canvas has the CSS transform (projector, and the controller's projected preview) it is drawn square via `identityProject` and the transform bends it; in the controller's flat preview there is no transform, so the geometry itself is bent by feeding `squareToQuad`/`projectUnit` in as the projector function. Bending the geometry rather than the canvas is what keeps pointer coordinates valid, so the handles stay draggable while the grid still shows the quad. A projective map takes lines to lines, so mapping each line's two endpoints is exact — no subdivision. The canvas container must stay unpositioned: giving it a `position` lifts it over the header popovers in paint order.

...
export function FontPickerModal({ currentFontId, onSelect, onClose }: FontPickerModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 sm:flex sm:items-center sm:justify-center">
      ...
      {/* Modal card */}
      <div className={`relative z-10 flex flex-col bg-slate-900 w-full h-full sm:h-auto sm:max-h-[70vh] sm:max-w-lg sm:rounded-2xl sm:mx-4 overflow-hidden shadow-2xl border-0 sm:border sm:border-slate-700/60 transition-all duration-200 ${visible ? 'opacity-100 sm:scale-100' : 'opacity-0 sm:scale-95'}`}>
        {/* Header */}
